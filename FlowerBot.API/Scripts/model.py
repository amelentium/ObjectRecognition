...
DATASET_DIR_PATH = '../Images/train_images/tiny-imagenet'
TRAIN_IMG_DIR_PATH = '../Images/train_images/tiny-imagenet/train'
VAL_IMG_DIR_PATH = '../Images/train_images/tiny-imagenet/val'
...
def model_create(class_count = -1):
    ...
    custom_fc = nn.Sequential(
        ...
        nn.Linear(int(feature_out_count), class_count if class_count > -1 else len(label_class_dict))
        )
...
def model_train(model, criterion, optimizer, scheduler, epoch_count=15, train_state={}):
...
def create_checkpoint(model, criterion, optimizer, scheduler, path='./model.tar'):
    ...
    checkpoint = {
                  ...
                  'class_count': len(label_class_dict),
...
def load_checkpoint(path='./model.tar'):
    ...
    model = model_create(class_count=class_count)
    model.load_state_dict(checkpoint['model_state_dict'])
    
    return model, class_count, criterion, optimizer, scheduler
